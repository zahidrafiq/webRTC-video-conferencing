...
function getMediaElement(e, t) {
    function E() {
        ...
        if (d) {
            ...
            if (v) v.style["border-top-right-radius"] = "50%"
        } else { l.style.marginLeft = i.clientWidth - l.clientWidth - 2 + "px" }
    ...
    if (!e.nodeName || e.nodeName.toLowerCase() != "audio" && e.nodeName.toLowerCase() != "video") {
        if (!e.getVideoTracks().length) { return getAudioElement(e, t) }
        var n = e;
    ...
    if (r.has("take-snapshot")) {
        ...
        s.appendChild(a);
        a.onclick = function() { if (t.onTakeSnapshot) t.onTakeSnapshot() }
    }
    if (r.has("stop")) {
        ...
        f.onclick = function() {
            i.style.opacity = 0;
            setTimeout(function() { if (i.parentNode) { i.parentNode.removeChild(i) } }, 800);
    ...
    if (r.has("full-screen")) {
        ...
        function m(e) { if (e.requestFullscreen) { e.requestFullscreen() } else if (e.mozRequestFullScreen) { e.mozRequestFullScreen() } else if (e.webkitRequestFullscreen) { e.webkitRequestFullscreen(Element.ALLOW_KEYBOARD_INPUT) } }

        function g() { if (document.fullscreen) { document.cancelFullScreen() } if (document.mozFullScreen) { document.mozCancelFullScreen() } if (document.webkitIsFullScreen) { document.webkitCancelFullScreen() } }
        ...
        function y(e) {
            ...
            if (t.height) { b.style.height = (n ? window.innerHeight - 20 : t.height) + "px" }
            if (!n && t.onZoomout) t.onZoomout();
            if (n && t.onZoomin) t.onZoomin();
            if (!n && v.className.indexOf("zoom-out") != -1) { v.className = v.className.replace("zoom-out selected", "zoom-in"); if (t.onZoomout) t.onZoomout() }
            setTimeout(E, 1e3)
        }
    ...
    E();
    i.toggle = function(e) { if (typeof e != "string") { for (var t = 0; t < e.length; t++) { i.toggle(e[t]) } return } if (e == "mute-audio" && o) o.onclick(); if (e == "mute-video" && u) u.onclick(); if (e == "record-audio" && c) c.onclick(); if (e == "record-video" && h) h.onclick(); if (e == "stop" && f) f.onclick(); return this };
    i.media = e;
    return i
}
...
function getAudioElement(e, t) {
    ...
    a.style.width = " 200px";
    if (t.buttons && t.buttons.indexOf("record-audio") == -1) { a.style.width = " 241px" }
    a.type = "range";
    a.min = 0;
    a.max = 100;
    a.value = 100;
    a.onchange = function() { e.volume = "." + a.value.toString().substr(0, 1) };
    u.appendChild(a);
    var f = document.createElement("div");
    f.className = "control stop";
    i.appendChild(f);
    ...
    if (t.showOnMouseEnter || typeof t.showOnMouseEnter === "undefined") {
        ...
        setTimeout(function() {
            p();
            setTimeout(function() { i.style.opacity = 1 }, 300)
        }, 700)
    }
    p();
    r.toggle = function(e) { if (typeof e != "string") { for (var t = 0; t < e.length; t++) { r.toggle(e[t]) } return } if (e == "mute-audio" && s) s.onclick(); if (e == "record-audio" && o) o.onclick(); if (e == "stop" && f) f.onclick(); return this };
    r.media = e;
    return r
}
